...
*Linux* binary is statically linked with https://www.musl-libc.org[musl libc], jansson, openssl, pcre and zlib from https://alpinelinux.org[Alpine Linux] {alpine-branch}.
It’s compiled with debug mode, threads and aio.

*macOS* binary is statically linked with jansson, openssl, pcre and zlib from https://brew.sh[Homebrew].
It’s compiled with debug mode, threads and aio.

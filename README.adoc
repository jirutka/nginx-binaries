...
await NginxBinary.download({ version: '1.18.x' }, '.tmp/')  // => '.tmp/nginx-1.18.0-x86_64-linux'

await NjsBinary.versions({ version: '^0.4.4' })  // => ['0.5.0']
await NjsBinary.download({ version: '^0.4.4', variant: 'debug' })  // => '.tmp/njs-0.5.0-debug-x86_64-linux'
----
